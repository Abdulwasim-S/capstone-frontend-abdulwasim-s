      ...
      onSubmit: async (loginInfo) => {
        try {
          ...
          const response = await fetch(
            "https://capstone-backend-abdulwasim-s.vercel.app/login",
            {
              method: "POST",
          ...
          if (data.message === "login success") {
            localStorage.setItem("markdown-token", data.token);
            localStorage.setItem("markdown-email", data.email);
            navTo("/markdownpage");
          } else {
    ...
    <div className="">
      ...
      <form className="text-start p-5" onSubmit={handleSubmit}>
        ...
        <div className="text-center m-3">
          ...
          <br />
          <br />
          <button onClick={demo} className="btn btn-success px-5">
            Demo Login
          </button>
          <span></span>
        </div>
